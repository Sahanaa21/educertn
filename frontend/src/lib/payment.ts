...
type VerifyAcademicServicePaymentArgs = {
	requestId: string;
	zwitchOrderId: string;
	token: string;
	attempts?: number;
...
export async function verifyAcademicServicePaymentWithRetry({
	requestId,
	zwitchOrderId,
	token,
	attempts = 6,
	intervalMs = 4000,
}: VerifyAcademicServicePaymentArgs): Promise<PaymentVerificationResult> {
	const normalizedRequestId = String(requestId || '').trim();
	const normalizedOrderId = String(zwitchOrderId || '').trim();
	const normalizedToken = String(token || '').trim();
	...
	for (let attempt = 1; attempt <= attempts; attempt += 1) {
		const response = await apiFetch(`/api/student/academic-services/${normalizedRequestId}/verify-payment`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				Authorization: `Bearer ${normalizedToken}`,
		...
		const isStillPending = response.status === 400 && lowerMessage.includes('not completed yet');
		if (!isStillPending || attempt === attempts) {
			return { verified: false, message: message || 'Payment verification failed' };
		}
