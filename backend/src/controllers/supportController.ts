...
export const updateIssueReportFromEmail = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const respond = (statusCode: number, params: Record<string, string>, fallbackHtml: string) => {
        ...
        if (!ISSUE_MAIL_ACTION_SECRET) {
            return respond(
                503,
                { ok: '0', issueId, status: action || 'UNKNOWN', message: 'Mail actions are not configured' },
                '<h2>Mail actions are not configured</h2><p>Please set ISSUE_MAIL_ACTION_SECRET and retry.</p>'
            );
        }
        ...
        if (!issueId || !action || !token) {
            return respond(
                400,
                { ok: '0', issueId, status: action || 'UNKNOWN', message: 'Invalid mail action link' },
                '<h2>Invalid mail action link</h2><p>Required parameters are missing.</p>'
            );
        }
        ...
        if (!EMAIL_ACTION_STATUSES.includes(action as (typeof EMAIL_ACTION_STATUSES)[number])) {
            return respond(
                400,
                { ok: '0', issueId, status: action, message: 'Invalid mail action' },
                '<h2>Invalid mail action</h2><p>Only IN_PROGRESS, RESOLVED, and CLOSED are allowed via mail.</p>'
            );
        }
        ...
        if (!valid) {
            return respond(
                401,
                { ok: '0', issueId, status: action, message: 'Mail action link expired or invalid' },
                '<h2>Mail action link expired or invalid</h2><p>Please use the latest email link.</p>'
            );
        }
        ...
        if (!existing) {
            return respond(
                404,
                { ok: '0', issueId, status: action, message: 'Issue not found' },
                '<h2>Issue not found</h2><p>This issue may have been deleted.</p>'
            );
        }
        ...
        return respond(
            200,
            { ok: '1', issueId, status: action, message: 'Issue status updated' },
            `
